...
**本页内容。** 如何精确确定你持有哪一款 CMP 170HX，从而确定适用哪个解锁档位。包括 `lspci` 设备 ID、子系统 ID、板卡和 GPU 料号、解锁前后 `nvidia-smi` 报告的内容、散热器下方的晶片标记、把两个 SKU 区分开的寄存器级指纹，以及 `install.sh` 在安装时运行的精确档位检测阶梯。最后是一张从"你观察到的"到"你得到的"的决策表。
...
注意类别：它是 `3D controller`，不是 `VGA compatible controller`，因为这张卡没有显示输出。`rev a1` 是 GA100 硅片修订，两者相同。
...
子系统 `10de:1585` 是 8 GB 卡。子系统 `10de:1557` 是 10 GB 卡。子系统 ID 是这张卡上**确实**由 VBIOS 设置的唯一标识，所以把它当作佐证而非证据。主设备 ID 熔进晶片：`FUSE_DEVID_SW_OVR_DIS`（`0x00820584`）在每张被探测的卡上都读 1，所以软件无法覆盖它，跳线电阻也改不了它。
...
`nvidia-smi` 不报告有用的产品名。两个 SKU 都显示为 **`NVIDIA Graphics Device`**，Linux 监控工具显示一条通用的 "NVIDIA display device" 条目是正常行为。容量字段和料号才是有用的。
...
>
> NVML 拒绝跨驱动版本通信，所以通过不匹配的 `nvidia-smi` 二进制读到的 `memory.total` 毫无意义。有一轮长达数天的测量系列正是这样失效的：用 580.159.03 的用户态去配另一个内核模块构建。如果 `nvidia-smi` 打印 "driver/library version mismatch"，先修好它再信任它说的任何话。参见[排障](../procedures/troubleshooting.md#version-mismatch)。
...
>
> GPC 地板清扫掩码按晶片而异，不按 SKU 而异。在两个 SKU 的 170HX 卡上观察到的值包括 `0x13`、`0x15`、`0x23`、`0x25`、`0x45`、`0x85`、`0xa8` 和 `0xd0`，而它们全都仍然枚举 70 个 SM。永远不要硬编码一个地板清扫值或据此推断 SKU。改用 `FUSE_SKU_ID` `0x00821060`（8 GB 卡上为 `0x80`，10 GB 卡上为 `0x68`）。
...
| HBM 堆叠 | **未解决。** 一张去盖的 8 GB 卡明显可见六堆；一张晶片照片来源声称六堆中有两堆是假的。两种读数都与实测总线宽度相容，所以总线宽度无法定论 | 报告 5 堆 |
...
| 显存时钟 | **未解决**，见下方警示框 | 1215 MHz，当前等于最大，无余量 |
| 解锁到 | **64 GB** | **40 GB** |

> [!NOTE]
> **未解问题：出厂 8 GB 显存时钟**
>
> 出厂 8 GB 显存时钟未解决：1458 MHz（一次扫描和 TechPowerUp）、1728 MHz（`nvidia-smi -q` 的 Supported Clocks，标注为 "432 MHz × 4"）、1890 MHz（解锁后的 64 GB 在 300 W 下跑 `gpu_burn` 时 `nvtop` 显示）。1215 MHz 是 10 GB 卡的，很可靠。一个貌似合理的调和方案——出厂 1458 / OC VBIOS 1728 / 超频后的 OC VBIOS 1890——尚未证实，而且 1728 MHz 这个数字被单独归因于 POST 时的 FWSEC devinit 而非 OC VBIOS，因为在八份转储的 ROM 里都没有出现 Memory Clock Table。一次原始的 FBPA PLL 读取就能定论它。

具体哪些 FBP 被地板清扫因卡而异。一份 10 GB 转储读 `FBP_DEFECTIVE` = `0x840`（FBP6、FBP11），与 A100 PCIe 40/80 GB 部件相同的模式，但该读数背后的卡身份有争议（一个来源把它归因于 8 GB 卡），而另一份 10 GB 探测读 `OPT_FBP_DISABLE` = `0x00000009`（FBP 0 和 3）。中等置信度，单一转储。10 GB 卡上每次干净的解锁触发都报告 `CSTATUS=20/24`。
...
但驱动内门 `_kgspSec2PostblTimingEnabled()` 只接受 **`0x20C2` 和 `0x2082`**。因此 `20b0` 卡会干净地安装却永不解锁。安装器会这么说并继续：
...
`0x20b0` 是 A100 SXM4 40 GB 的设备 ID，也由一个 A100 工程样品携带（8192 MB、2048-bit、4096 个 CUDA 核心、Samsung 8Hi HBM2）。README 里较旧的 "unlock is `0x20C2`-gated" 措辞已经过时：自 "Unlock isn't gated anymore" 提交起，`0x2082` 一直是头等目标。

两条相关备注：

* 补丁 0001 和 0002 中每条 `SEC2_DEBUG` 打印都门控在这两个相同的设备 ID 上，所以在 `20b0` 卡上的出厂构建应该**什么都不打印**进 `dmesg`。有一份关于 `20b0` 工程样品上出现 SEC2_DEBUG 行的报告未解决，最有可能是修改过的构建或同一主机里的第二张卡。
* `0x20BB` 是 Drive A100 / PG199 部件（出厂 32768 MiB）。名为 `PG199` 的分支**什么**都没加：它的树与 `ecc` 分支逐字节相同，提交列表为空，其中没有任何地方提到 PG199、`0x20BB` 或 A100D。它不增加检测，不含 A100D 支持，也不改动 `lspci` grep 或驱动内门。不存在 PG199 解锁。
...
> **在混合 GPU 主机上自动检测不安全**
>
> `detect_card_profile()` 取 `nvidia-smi` 顺序里的**第一张 GPU**，它不一定是 `lspci` 找到的那张 CMP。一台 RTX 3080 10 GB 与 8 GB CMP 170HX 并存的主机上，至少有两人复现出从 3080 检测出 "10GB"；还有一份独立报告说一块 CMP 50HX 被误检为 10 GB 170HX。如果第一张 GPU 报告的大小落在四个窗口之外，例如 24 GB 卡，安装会直接死掉。
>
...
| `[10de:20b0]` | A100 SXM4 40 GB 或 A100 工程样品 | 安装，警告 | **不变** | 无；驱动内门拒绝它 |
...
> **80 GB 那一行不是受支持的选项**
>
> 归档的 `80` 分支把 10 GB 卡编程为报告 81920 MiB，但卡在超过约 40 GB 就不可用：几秒钟内挂起、Xid 154 和老化测试错误。每个提到它的来源都把其标记为不稳定或被否决，而且它与功耗上限无关。它实际编程的是一个三处不一致的组合（CFG1 `0x02779000` 配 LMR `0x0000028A` 和 80 GiB 的 `fb_length`），这本身很可能就是原因。参见[80 GB](../frontier/80gb.md)。
...
* [Risks](risks.md)：开始前请阅读
* [Quick start](quick-start.md) 和[安装](../procedures/install.md)
