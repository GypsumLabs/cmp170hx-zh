...
**本页覆盖内容。** 当一张 CMP 170HX 无法初始化时如何把它弄回工作状态：从最便宜到最激烈的复位阶梯、冷启动做了热重启不做的事、FLR 清除什么、只有次级总线复位（SBR）清除什么、每种复位恰好留下哪种寄存器状态、如何移除打过补丁的模块并恢复出厂系统，以及有多少真实变砖风险的一次诚实评估。

**短答案。** 解锁写寄存器。它不写熔丝、不写 VBIOS、不写 EEPROM、在出货工具上也不写固件文件。它碰的每个几何寄存器都是易失的、断电即回退，这意味着**一张无法起来的卡几乎总可以通过断电恢复**。整个语料库里从未确认过永久变砖。一份一手报告与无持久状态模型矛盾、在下文[变砖风险](#变砖)里被诚实记录；它仍未解释。
...
第 5 级在实践中是主要恢复工具。记录的一手建议很直白："lots of cold boots required from the way this gets wedged"（按它被卡死的方式需要大量冷启动）、并建议关机时把 PCIe 电源从卡上拔掉。

**为什么阶梯既有 FLR 又有 SBR。** FLR 复位引擎却**不**复位 AON 岛，所以根植于 AON 临时区的一个 `0x65` 卡死挺过 FLR、而 SBR 清除它。见[FLR 对比 SBR](#flr-对比-sbr)。
...
**热重启不是一次复位。** 它让 WPR2 保持 up、板卡电容保持充电。每个已发布流程里、一次失败解锁后要求的 "a reboot" 都指**冷**的，而这个区别是承重的：同一个挺过五次 `reboot` 循环的症状、经常在第一次真正的断电循环上清除。
...
60 秒等待在已发布指南的每个阶段都被规定为硬要求，而且它与出货补丁的 WPR2 保存-恢复行为一致。同一个流程对 PLM-not-opened 和 VRAM-still-8-GB 两个症状都被规定。
...
FLR 抹掉 PL0 临时区写并**重新感测熔丝**。一次普通 `modprobe` 重载不抹配置空间写；FLR 抹。这用 `0x14A0` 处的 PL0 scratchpad 作代理测试、并被利用的原始作者佐证。
...
**为什么 FLR 有时无法恢复一个 `0x65` 卡死。** `SECURE_SCRATCH_14`（`0x001180f8`）住在 PGC6 "GC6 island" 常电功率域、被标为 RW-4R（priv-masked）。AON 临时区挺过引擎复位和 FLR，所以一个 un-DONE 的交接加让 Booter 自己的对 `0x1180f8` 的 DIO 读返回 `0xdead5ec1` 的毒化 PLM 和特权状态、径直挺过 FLR。SBR 掉电并重新初始化 AON 功率域、清除临时区、让一次新鲜的 Booter 能跑第 3 阶段并自己设 DONE。
...
*算力先于显存出货就是因为 FLR 不对称。* SS0、SS1 和特性覆盖 PLM 坐在常电岛里、挺过 FLR；整个显存几何布局不挺。这就是旧 FLR 系流水线能跨复位解锁算力、却每次丢掉几何布局的原因。也是出货补丁在**一次** GSP 引导里、中间不带复位地打开 PLM 并写几何布局的原因。见[显存几何布局](../unlock/memory-geometry.md) 和[算力节流](../unlock/compute-throttle.md)。

*解锁写的任何东西都不挺过断电循环。* 一张断电的卡回来是出厂状态。那是下面变砖评估背后唯一最重要的一个事实。
...
`setpci` 步要紧。`0x0546` 设了位 2（Bus Master）；`0x0102` 没有，而一张总线主控关闭的卡在独立工具朝它发射时会静默什么都不做、日志里任何地方都没有 DMA 错误。完整失败模式见[总线主控被清除](troubleshooting.md#bus-master)。
...
nvidia 模块常无论如何都拒绝卸载、留下 `nvidia 15835136 2`、`drm` 被包括 `i915` 在内的七个用户持有。那条依赖链是为什么要在一台无头或非 NVIDIA 显示的主机上做解锁工作的实用原因。
...
一张 10 GB 卡曾困在一个不可中断睡眠状态、挺过约五次冷重启、并阻止 Ubuntu 关机。**原因是自动加载的补丁内核驱动、不是卡。** 让卡断开引导并清理解决了它。*（置信度：中等；受影响测试者在恢复后识别了根因。）*
...
`remove.sh` 没有 `--yes` 或 `-y` 拒绝运行。它跑五步、写 `logs/remove_YYYYMMDD_HHMMSS.log`、仓库目录不可写时回退到 `/tmp`。它做什么：

* 停止显示管理器、`modprobe -r` 失败时强制 `rmmod`。
* 停止、禁用并删除遗留 `/etc/systemd/system/cmpunlocker.service`、杀掉任何遗留 `/opt/cmpunlocker/daemon/watchdog.py` 进程。两者都是一个当前安装器从不创建的废弃看门狗设计的遗存。
...
一个被删除或不匹配的 `/lib/firmware/nvidia/<version>/` 目录产生一个看起来像渐进硬件退化的失败。一次无法复现的多日 "model degradation" 结果是一个被删除的 `/lib/firmware/nvidia/580.159.03/{gsp_tu10x.bin, booter_*.bin}` 加一个不会在一个 `.03` 模块上触发 GPU init 的 `.04` 用户态 `nvidia-smi`。那种状态下 `SEC2 MBOX0 = 0x0` 意味着 Booter 根本没加载。恢复版本匹配的固件目录立即复现先前工作状态。

保留任何驱动修改的 diff 或变更日志。重装一个新驱动静默丢弃每次需要的注入。

### 4.5 回到一张出厂、从未碰过的卡 { #restore-stock }

硬件里没有任何要撤销的东西。解锁是主线上 NVIDIA 驱动之上的一个补丁、不是固件替换，所以一旦打过补丁的模块消失、机器已被断电循环，卡逐位就是开始时的卡：CFG1 `0x02449000`、LMR `0x00000208` 或 `0x00000288`、每-FBPA `CSTATUS_RAMAMOUNT` `0x200`、SS0 回到它的锁定值。
...
* 不刷 VBIOS 或任何 EEPROM；
* 在出货工具上不修改 `/lib/firmware` 下任何文件（更早的固件打补丁一代确实改、这正是 `remove.sh` 仍为它清理的原因）；
...
4. **让卡跑在稳定几何布局之外。** 8 GB 卡在 64 GB 稳定且在产；10 GB 卡在 40 GB 稳定；10 GB 卡在 80 GB 报告容量、超过约 40 GB 却不可用：触碰更多的内核造成致命 GPU 丢失、与功耗上限无关。报告的 Xid 码包括 Xid 31（被描述为无害）和 CUDA 显存测试后的 Xid 154；主导报告症状是挂起、连同老化测试错误。Xid 31 单独是一个旁观者提出的、并未被带故障卡的操作者佐证为*那个*签名。它毁工作负载、不毁卡。见[80 GB](../frontier/80gb.md)。
