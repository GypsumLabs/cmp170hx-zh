...
解锁在这里无关紧要。对已发布的解锁器和全部 12 个未发布分支做 `thermal`、`pstate`、`power_limit`、`vid_pwm`、`clkdomain`、`freqDelta`、`0x20340`、`MHz` 和 `watt` 的关键字扫描，在任何补丁、脚本或配置里都一无所获。下面每一个热特性都是出厂板和出厂 VBIOS 的属性。
