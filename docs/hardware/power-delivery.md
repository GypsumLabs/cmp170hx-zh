...
这些地址在已发布的解锁器或 12 个未发布分支的任何一个中都**从不出现**，经关键字扫描验证。

---

## 解锁根本不碰供电

对已发布的树和全部 12 个未发布分支快照做 `0x20340`、`0x20344`、`freqDelta`、`power_limit`、`powerlimit`、`thermal`、`pstate`、`vid_pwm`、`clkdomain`、`MHz` 和 `watt` 的关键字扫描，在任何补丁、脚本或配置里都一无所获。解锁只碰权限级别掩码、SS0/SS1、CFG1、LMR 以及 GSP 帧缓冲/PMA 描述，不碰其它任何东西。
