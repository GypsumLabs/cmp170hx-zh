...
**本页覆盖内容。** 整个 CMP 170HX 解锁运行其上的安全协处理器：SEC2 Falcon 是什么、"Booter Load" 微码做什么、重度安全模式如何进入和离开、booter 映像在其两个地址空间内的内部布局、GSP 签名缓冲区坐在哪，以及打过补丁的驱动究竟如何调用 booter。利用本身在[ROP 链](rop-chain.md)；它打开的掩码在[权限级别掩码](privilege-level-masks.md)。

**两句话的关键结果。** NVIDIA 自己签名、AES 加密的 `booter_load` 微码被正常加载和认证，然后在一个认证*已经*成功后、被一个主机驱动控制的签名缓冲区破坏。没有伪造签名、没有提取密钥、也从不执行任何攻击者提供的指令：booter 变成解锁的执行引擎，同时逐字节保持为 NVIDIA 发货的微码。
...
因为 GSP RISC-V 核心没有任何密码功能，它无法验证自己的映像。验证被委托给一个叫 *booter* 的 SEC2 Falcon 微码。存在两个 booter，`booter_load` 和 `booter_unload`；本页讲 `booter_load`。

booter **不**是[VBIOS](../hardware/vbios.md) 的一部分。它作为 `nvidia.ko` 里一个编译进的 BINDATA 数组随驱动包发货，而且它是按版本的，正如它验证的加密 GSP 固件。在驱动 610 里 GA100 数组是：
...
因此运行利用不需要单独一个 booter 文件。驱动已经带着它。

解锁攻击的、端到端的引导链是：上电时从 SPI 闪存加载 VBIOS、一颗晶片上 Falcon 验证 VBIOS、芯片进入安全模式、驱动提供 `gsp_tu10x.bin` 而其签名被 Falcon 检查，然后驱动用 GSP 客户端读显存容量并暴露设备。解锁攻击第四步，在 booter 要 DMA 的 GSP 签名缓冲区里植入一个载荷。
...
项目的 "L0 到 L3" 权限级别词汇指同一个模型。这些级别如何按寄存器被强制执行见[权限级别掩码](privilege-level-masks.md)。

`booter_load` 永远不能在 NS 模式执行。它的主体是 AES 加密的，只能在 HS 模式的 Falcon 内被解密。Falcon 通过运行 0x100 字节明文 NS 前导、然后发出那条解密代码、验证它并切换到 HS 的特殊指令来进入 HS。后果是架构性的，也是整个解锁被塑造成现在这个样子的原因：
...
零的开始或大小触发 `OP_SECURE_FAULT`。要认证必须有四个条件：微码页必须映射在一个预先选择的虚拟地址、标为机密、该信息载入 `SEC` 寄存器，且密码寄存器 6 里有一个有效 MAC。

> [!WARNING]
> **两种不同的验证方案，不要混为一谈**
>
> 不可变引导 ROM 对 HS booter 映像的检查在语料库里被描述为一次 RSA-3K 检查，而一个 384 字节签名块确实随映像在 `PATCH_LOC = 0x8900` 发货。分开地，booter *自己*对它加载的 GSP 映像的验证被追溯到 `_acrVerifySignature_TU10X` 到 `_acrCalculateDmhash_TU10X` 到 `_acrDeriveLsVerifKeyAndEncryptDmHash_TU10X` 到 `_acrMemcmp`，那是 Davies-Meyer 哈希加一次从一个 `csecret` 派生的 AES 密钥派生，那条路径上没有 RSA。booter 确实含一个单独在别处用的 PKA/modexp 块（`rsa_pubkey_load 0x4768`、`pka_modexp_run 0x54ab`）。对调和的置信度为中等：档案里没人用恰好这些术语陈述过它。
...
因为 GSP 签名缓冲区活在 DMEM 里，签名 DMA 只能粉碎 DMEM、永远不能写 IMEM。那单一事实就是为什么利用是对一个已签名映像的纯返回定向编程、而非代码注入。它也意味着 DMEM 的 16 位空间本身无法寻址一个 32 位 BAR0 寄存器，所以载荷需要一个驱动 Falcon BAR0 master 的 gadget。
...
- **BAR0 / PRI**，主机对特权寄存器接口的 32 位视角。主机、SEC2 和 GSP RISC-V 都经 PRI 说话；PLM 门控它的区域。

---

## 4. 从主机看到的 SEC2：BAR0 寄存器图

SEC2 坐在 `NV_PSEC_BASE = 0x00840000`、是一颗 Falcon 核心而非 RISC-V（`HWCFG2` 位 10 读 0）。完整偏移量，由一台 GPU `10de:20c2` 上一个能工作的无驱动 C 加载器验证：
...
未压缩代码大小是 `0x8700` = 34,560 字节。分析的调试签名构建的实测段大小是 34,304 字节安全代码、25,600 字节数据和 256 字节非安全代码。原始提取的 GA100 文件被零填充到 60,100 字节；编译进的 BINDATA 块是 60,160 字节。
...
如果上面那个 16 字节常量恰好出现在文件自己的零区域之前，调试块被正确提取并正确对齐；如果它用测试密钥工具解密回零，解密是对的。量产块显示不同的尾部模式。一个错过会花掉数小时的工作流细节：4 字节轮密钥必须按与人类可读形式**相反**的顺序喂给 AES/Rijndael 工具，因为密钥编号坐在最后一轮密钥而非第一轮（置信度：中等，单条可操作指令，但它描述的工作流显然产生了正确输出）。

### 5.3 提取

提取是通过打补丁 NVIDIA 自己的 `extract-firmware-nouveau.py` 只发 GA100 量产和调试 booter 完成的，解析 `kgspBinArchiveBooter{Load,Unload}Ucode_GA100_BINDATA_LABEL_IMAGE_{PROD,DBG}_data` 和匹配的 `..._SIG_{PROD,DBG}_data`。**GA100 签名大小是每个签名 384 字节**，TU10x 用 16，所以调用是 `booter('ga100','load',384,'prod')` 和三个兄弟。所有非 GA100 芯片（tu102、tu116、ga102、ad102、gh100、gb100、gb202）都被从 `main()` 注释掉。

第二个变体 `extract-firmware-nouveau-ga100-raw.py` 剥离所有容器结构（带 `0x10de` 魔数和 6 dword 的 `nvfw_bin_hdr`、带 9 dword 的 `nvfw_hs_header_v2`、签名块、`patch_loc` / `patch_sig` / `fuse_ver` / `engine_id` / `ucode_id` / `num_sigs` 和描述符），只把原始固件写到 `booter_{load,unload}_{prod,dbg}-<ver>_raw.bin`。那个原始布局喂给 envydis 和 objdump，是以后所有工具链的输入。

第二条提取路径从一个已加载的出厂驱动工作：用设了位 25 自动递增的 `IMEMC 0x840180` 和用于偏移量 `0`..`0x8700` 的 `IMEMD 0x840184` 读 SEC2 IMEM，用 `DMEMC 0x8401c0` / `DMEMD 0x8401c4` 读 DMEM，然后拼接 IMEM(NS+HS) + DMEM。

Booter 几何布局也可以**不用反汇编器**恢复，通过扫描原始映像的前 `0x100` 字节：`imm_before(ns, ff9f04)` 给出 NS 末尾、`imm_before(ns, fd9e04bb9002b69410)` 给出 DMEM 偏移量，其中 `imm_before` 要求标记前四个位置处的字节是 `0x89`（`mov $r9 imm24` 操作码），然后组装小端 24 位立即数。如果任一标记缺失、或 `dmem <= base`，解析器抛 "not an ACR booter image"。加载随后把 `img[0:ns]` 非安全地写到 IMEM 0、把 `img[ns:dmem]` 带置位的 SECURE 位 `1 << 28` 写到 `IMEM[ns]`、把 `img[dmem:]` 写到 DMEM 0。
...
已发表论文的跨版本语料跨越分支 450、460、470、510（两个点发布）、515、525、535、560、570 和 580。510 SEC2 booter 的签名路径只用常量或钳制的 DMA 长度、没有元数据大小的复制；580 booter 表现出无界复制；525 映像无法恢复，因为 booter 打包变了。

> [!NOTE]
> **未解问题：第一个受影响的分支**
>
> 溢出在 **510 中缺失**、在 **580 中存在**。分支 515 到 570 **不确定**。通过恢复并分析 515、535、560 和 570 的 GA100 booter 是否含元数据大小的复制来定论。

### 5.5 谱系命名

GA100 用 **Turing 代**固件：GSP 块是 `gsp_tu10x.bin`、SEC2 booter 是 Turing 谱系的 `booter_load`。这在 NVIDIA 自己的树 `nouveau/extract-firmware-nouveau.txt` 里被陈述，并被失败路径上 TU102 后缀的 RM 符号佐证（`kgspBootstrap_TU102`、`s_executeBooterUcode_TU102`）。一次频道内更正后的正确命名：
...
1. 在 `0x0107` 清理 SCP 状态（`csigclr` / `csecret` / `cxor`）。每个 `csecret $cN 0x0` 立即被 `cxor $cN $cN` 跟随，所以寄存器组在被配置的那一刻就被清零。
...
6. **阶段 6** 按序碰 CSB `0x4e00`（掩码 `0xff000000`，然后 OR `0x80003000`）、`0x10100`（OR `0x101`，旋转到位 `0x100` 清除，以 `0x400` 次迭代为界）、`0x14000`（设 `0x7fff`）、`0x14100`（保留低 16 位，OR `0x03ff0000`）、`0x14b00`（OR `0xff00`），然后又 `0x10100`（OR `0x1000`）。
7. 经 `crnd` 在 `0x0433` 的 SCP 自配置。
...
在 MAIN.2、紧接 `watchdog_set` 之后，Booter Load 在 **CSB** 空间写四个固定权限级别掩码和孔径值：
...
每个都被内联的 fail-closed 断言跟随，所以四个中任何一个的 CSB 错误都会把 Falcon 楔死在无限自分支里。
...
**DMEM `0xFFEC` 是喂退出状态并决定是否运行 teardown 的槽。** 这由一次硬件 A/B 定论、反驳了一个竞争的 `0xFFE4` 假设，并与已发表的 ROP v3 注释 "FFEC 00000000 <- Return value to main() to indicate success ($r0)" 匹配。把 `D[0xFFEC]` 设成 `0xDEADBEEF` 让 `0x001180f8` 从 `0x11000000` 移到 `0xf1000000`，恰好符合 `(r0 << 28)` 模型预测。
...
这里所有地址都是 **DMEM**。`0x100` 之下什么都不分配，这就是为什么 "staged in low DMEM 的 mega-ROP" 被排除。
...
每次启动从硬件 RNG 生成一个新随机值，保存在 DMEM `0x6340` 的全局里。每个受保护函数把它复制到栈帧边界并在退出时重读比较；不匹配调用 `0x7dd9` 处的 `panic()`。规范序言是 `mov $rX 0x6340; ld b32 $r9 D[$rX]`；尾声是 `cmp b32 $r15 $r9; bra e <ok>; lcall 0x7dd9`。

因为该值每次启动从硬件 RNG 重新生成，它无法离线猜测。它也不需要被猜。守卫全局活在可写数据内存里、恰好被它要检测的那个溢出可达，所以载荷用同一个选定值覆写全局**和**每个重建的金丝雀槽，每个尾声比较都通过。这是已发表论文的 Thesis 1：Falcon 栈金丝雀在 *引用字完整性* 上失败，而非熵。在这个映像里工具链在只读数据段的尾部发出守卫，而在扁平 MPU 映射的映像里那落在可写数据跨度内。没有 RELRO 等效、没有守卫页、没有 MPU 只读映射。
...
| `I[0x1c200]` | 数据。对读，结果回到这里。 |
| `I[0x1c000]` | 命令：**`0x800000f2` = 写，`0x800000f1` = 读** |
| `I[0x1c300]` | 看门狗，被 `watchdog_set (0x1034)` 用 `0x1312d00`（20,000,000）播种 |

booter 用它自己的事走这条路径：`0x29b8 -> 0x10aa` 写 WPR2 寄存器，而 finalize 例程对 `0x001180f8` 的写字面上是 `1c4b: r10=0x1180f8 ; lcall 0x10aa`。一个对应的读出现在 `1c35: r10=0x1180f8 ; lcall 0x1196`。

### 9.2 Fail-closed CSB 访问

**booter 里每次 CSB 访问都是 fail-closed 的。** 每次访问后代码采样 CSB 错误状态 `I[0x9100]` 位 31，即 `FALCON_CSBERRSTAT.VALID`，一个**故障标志**、意思是上次 CSB 访问出错了，**不是**忙或完成轮询。原始内联序言出错时分支到自身、把 Falcon 永远楔死；两个辅助函数改为报告状态 `0x15` 并退出。
...
那个惯用语内联出现约 25 次加两个辅助函数。`csb_read (0x8264)` 还额外用 `0xffff0000` 掩码返回的数据并测试 PRI 毒哨兵 `0xbadf0000`，带一个白名单（`0x208c` 处的 `reg_whitelist_40f00`，覆盖 `[0x40f00, 0x41f00)` 步长 `0x100`）和一条对寄存器 `0x1c200`、`0xc00`、`0xb00` 和 `0xd500` 的重试路径。
...
>
> MAILBOX0 读 `0x31` 只意味着 `report_status` 从未执行。booter 自己在 ucode 偏移量 `0x7a` 印 `0x31`（`mov $r15 0x31 / mov $r9 0x1000 / iowrs I[$r9] $r15`）作为它第一个活性标记，覆写驱动种下的 WprMeta 物理地址参数。
...
IMEM `0x7e76` 处的 `secure_teardown` 是设计的退出。它重新启用 `0x10100` DMA 孔径（OR `0x101`，旋转到位 `0x100` 清除，以 `0x400` 次迭代为界）、设 `$cauth |= 0x80000`（位 19，停机前抑制中断和异常），然后对 `$c0`..`$c7` 各发出 `csecret $cN 0x0` 接 `cxor $cN $cN`、用 `r14 = 0` 从 0 到 `0x10000` 循环 `st b32 D[$r9] $r14; add $r9 0x4`、清除 `r0`..`r15`、并执行原始 `exit` 操作码（`f8 02`）。它永不返回。那个 `exit` 是让 Falcon 掉出 HS 模式、允许加载新代码的东西。

从 `main` 到达它需要 `0x8113` 处的 `r0 == 0` 分支到 `0x8119`。若 `r0` 非零，取 `0x8117 exit`、根本没有 teardown。

**错误路径总是按此顺序调用 `report_status (0x1d0f)` 然后 `secure_teardown (0x7e76)`**：这对出现在 `0x873`/`0x877`、`0x88a`/`0x88e` 和 `0x8a7`/`0x8ab`。成功路径两者都不调用，为 GSP 移交留下密码和环境完好。因此一个流传的 "mailbox XOR teardown" 框架是错的：错误两者都做。
...
关键是，`0x8f` 是 **在 HS 到 NS 退出转变时硬件锁存的**，不由任何 booter 指令写：静态分析在 booter 里发现零条对 `0x8403C4` 的指令引用。离开 HS 让硬件把每个 HS 门控的 PLM 重新保护到安全默认值。实测：在 `0x8117` 取原始退出留下 resetPLM `0xff`；让 `secure_teardown` 运行重新锁存到 `0x8f`。

> [!NOTE]
> **未解问题：`resetPLM = 0x8f` 会阻止加载新 SEC2 ucode 吗？**
>
> 一份报告说 SEC2 在 `0x8f` 时可重载（Hello World 触发、MAILBOX0 从 `0x0` 变 `0x31`）；另一份说加载新 ucode 需要 SFTRESET、而复位 PLM 门控它，并报告 `NS load mismatch (HS-locked, needs --flr)`。可能的调和——NS 重载有效、HS 签名重载无效——被提出但从未定论。一次在一个已知 `0x8f` 下背靠背加载一个 NS ucode 和一个 HS 签名 ucode、记录每个的 `CPUCTL` 和加载器错误字符串的受控实验能回答它。

出货驱动完全绕开这套纪律：它**从不读或写 `0x008403C4`**。对出货仓库 grep `0x008403c4`、`0x001180f8`、`0x001fa81c` 或 `0x001fa820` 发现零引用。驱动内路径改走驱动自己的 `kflcnReset`/FWSEC 序列重发 Booter Load，补丁 `0002` 通过记录 `SEC2_DEBUG: kflcnReset for FWSEC: 0x%x` 和 `SEC2_DEBUG: kflcnResetIntoRiscv: 0x%x` 确认它。
...
booter 逐字从 `WprMeta.sizeOfSignature` 取复制长度、没有任何边界检查，而驱动同时控制缓冲区内容和那个字段。放大是一个 15.5 倍因子。出厂签名的 DMA 只到达 DMEM `0x17FF`，这就是正常引导把 `0x2383` 和 `0x8E08` 处的寄存器描述符表保持完整的原因。
...
> **不是矛盾**
>
> `kernel_gsp_booter.c:329` 计算 `pUcode->hsSigDmemAddr = patchLoc - pUcode->dataOffset`，带 `patchLoc = 0x8900` 和 `dataOffset = 0x8700` 会把一个签名放在 DMEM `0x200`。那是 **booter 自己的 HS 签名**，加载前被打进 booter 映像。DMEM `0x800` 是 booter 从 sysmem DMA 的 **GSP-RM LS 签名**落下的地方，而那才是溢出的。两个不同的缓冲区。置信度：中等，因为这个调和与每个观察一致、但没人明确陈述过它。

还有一个属性对解锁的持久化故事要紧：**出厂 AES-MAC 签名在几何改动后仍有效**，因为它覆盖静态 GSP 固件映像、而非运行时 WPR 元数据或硬件几何布局。WPR 元数据由驱动在运行时计算。一条相反方向的早期说法被其作者明确撤回。
...
`_kgspSec2PostblTimingEnabled()` 把 `pGpu->idInfo.PCIDeviceID >> 16` 与恰好这两个值比较。一张 `install.sh` 也 grep 的 `10de:20b0` 卡会安装却不解锁。目标驱动版本是 `610.43.03`（默认）和 `610.43.02`；构建在其它任何版本上硬性失败。

### 12.2 按顺序的序列

1. **`_kgspCreateSignatureMemdesc`** 把签名 memdesc 分配在 `0x0000f800` 而非 `NV_ALIGN_UP(pGspFw->signatureSize, 256)`。改作它用之前，出厂签名字节被复制进 `pKernelGsp->pStockSignatureData` / `stockSignatureSize`，即加到 `g_kernel_gsp_nvoc.h` 里 `KernelGsp` 的两个新字段。记录 `SEC2_DEBUG: saved stock signature (%llu bytes)`，在卡上报告 4096。
2. **可选外部载荷。** `os_open_and_read_file()` 尝试把 `SEC2_POSTBL_TIMING_DMEM_PATH = "/lib/firmware/nvidia/ga100/gsp/dmem.bin"` 读进新缓冲区。成功记录 `SEC2_DEBUG: loaded %llu bytes from %s`；缺失记录 `SEC2_DEBUG: %s not found (0x%x), using built-in payload` 带 `0x59`，并回退到预置了 `writeAddr = 0x009a0148`、`writeValue = 0xffffffff` 的内置载荷。无论哪种情况都调用 `memdescFlushCpuCaches()`。
3. **保存 WPR2。** 读一次 `0x001fa824`（低）和 `0x001fa828`（高），记录 `SEC2_DEBUG: saved WPR2 lo=0x%08x hi=0x%08x`。
4. **PLM 循环。** 对四个表条目的每个、最多两次尝试：
    ...
    失败记录 `SEC2_DEBUG: FAILED to open <name> after 2 attempts`。表和它的精确值在[权限级别掩码](privilege-level-masks.md)。
...
这就是为什么 GSP-RM 在*同一*次驱动加载里正常引导：利用和真实引导在一次加载内顺序发生，不需要冷启动等效移交。
...
缓存刷新不是可选的。签名 DMA 是非连贯的，没有显式刷新 Falcon 会读陈旧的 RAM。
...
- **WPR2 已 up。** 致命路径
    ...
    变成 `NV_PRINTF(LEVEL_WARNING, "WPR2 already up before GSP boot; continuing for recovery\n")`。重复的 booter 发射让 WPR2 保持 up，每次发射重新划分它。空/INIT 状态是 LO `0x0fffffff`、HI `0`，而 **HI = 0 让 `kgspIsWpr2Up()` 返回 false**。出货驱动恢复*保存的*对、而非写一个空区域。
...
> **寄存器回读是唯一有效的成功标准**
>
> 每次载荷执行都记录 `s_executeBooterUcode_TU102: Booter failed with non-zero error code: 0x31` 和 `kgspExecuteBooterLoad_TU102: failed to execute Booter Load: 0xffff`，**而寄存器写入仍然落地**。在 `s_executeBooterUcode_TU102` 里 seccode 错误在每次运行后坐在 MAILBOX0、`mailbox0 != 0` 返回 `NV_ERR_GENERIC`（`0xffff`）。出货循环的成功测试是精确的回读相等，那是正确的测试。项目 README 说同样的事：早期 PLM 轮次期间的 `0x31` / `0xffff` 等 Booter 状态码，若最终引导成功则往往无害。

见[验证](../procedures/verify.md) 和[排障](../procedures/troubleshooting.md)。

## 13. 无驱动调用，作对比

一族独立 Python 和 C 工具（`refire_chain_v2.py` 到 `v9.py`、`load_gsp_sec2_falcon.c`、`load_custom_bin.py`）在没有任何 NVIDIA 驱动的情况下发射 booter。它们**不在**出货仓库里，而本领域大部分表面矛盾在把两个代码库分开后消解。工具要紧，因为寄存器纪律大多是在那里学到的。见[工具谱系](../history/tool-lineage.md)。

在 SEC2 上运行非安全代码是琐碎的、除公开文档外什么都不需要：把二进制 DMA 进 IMEM、设 `BOOTVEC`、发 `STARTCPU`、然后如果代码用 `exit` 就轮询 `CPUCTL` 位 4（HALTED）。完整无驱动引导序列是：引擎复位、等 DMA 清理、`ALLOW_PHYS_NO_CTX`、物理 DMA 孔径、DMA IMEM + DMEM、设 `BOOTVEC`、设邮箱、`STARTCPU`、轮询 HALTED。
...
默认超时 10.0 s；失败路径报告 "scrub timeout"。Booter 加载随后用 IMEMC/IMEMD 带自动递增和每 256 字节 IMEM 标签，对 HS 区域置位 SECURE 位 `1 << 28`。孔径被强制为物理模式，通过在 `0x00840600`/`0x604`/`0x608` 写 `FBIF_TRANSCFG[0..2] = 4, 5, 6`、然后在 `0x00840624` `FBIF_CTL |= 0x80`，在 `start_wait` 之前、MAILBOX0/1 被设为 WprMeta 物理地址的低和高半。
...
- **缓存刷新。** 一个 17 字节 JIT 组装的 x86-64 桩（`0F AE 3F 48 83 C7 40 48 83 EE 40 7F F3 0F AE F0 C3`，即 `clflush [rdi]` / `add rdi,64` / `sub rsi,64` / `jg` / `mfence` / `ret`）被映射 `PROT_EXEC` 并在载荷、radix3 和 WprMeta 缓冲上运行，向上取整到 64 字节缓存行。`refire_chain_v6.py` 用 `MAP_HUGETLB`（`0x40000`）分配 2 MiB 巨页、mlock 它们，并检查存在位然后 `(entry & ((1<<55)-1)) * 4096` 从 `/proc/self/pagemap` 解析物理地址。
...
> **无驱动发射能移交给出厂驱动吗？**
>
> 出厂驱动通过经典的两次加载 "mutex horns" 拒绝发射后的 SEC2 状态：`0x31`（互斥锁持有）、`0x62`（WPR2 up）和 `0x29`（一个 `0x001180f8` 错误，因为 `mutexfree` 终止符留下 `0xf0000000`、`0xf` 顶半字节触发检查）。这在几何一致时甚至在 10 GB 也失败，证明被发射扰动的是 SEC2 / `0x001180f8` 移交状态，不是几何布局也不是写次数。提出两个修复：让终止符把 `0x001180f8` 顶半字节留零，或从打过补丁的驱动内部分阶段做几何布局。**出货解锁器选了第二个。**

> [!NOTE]
> **不带 FLR 跨过 RmInitDone 墙**
>
> `whole_stack_rejoin` 终止符在利用后不带 FLR 重启 SEC2、让 booter 完成、GSP-RM RISC-V 核心启动，但 init 从不完成。这是同一个 `0x65` 引导墙。`0x001180f8` 是 `NV_PGC6_BSI_SECURE_SCRATCH_14`，位 26 是 `BOOT_STAGE_3_HANDOFF`（INIT = 0，DONE = 1），只有 HS 里的 SEC2 设它。预写 DONE 没用：读路径被 PLM 下毒所以 `0x001180f8` 回读 `0xdead5ec1`，而一次下毒读上位 26 已经读成 1、产生一个假 DONE、之后反而杀死 GSP-RM。两个候选根修复是保留 booter 成功路径让 SEC2 启动它的 RTOS 并自己设 DONE，或恢复 AON `SECURE_SCRATCH` PLM/priv 状态，而那今天只能靠一次功率域复位达成。

> [!NOTE]
> **移植到其它 CMP 卡**
>
> CMP 50HX 是 TU102、用一套完全不同的显存访问控制寄存器。CMP 90HX 是带 10 GB GDDR6X、没有额外物理显存的 GA102，所以只有算力解锁才有意义。既定规则是同一个 Turing booter、脚本和利用适用于任何 SEC2 接受 Turing 代 AES 和 RSA 密钥的卡。一位测试者报告 TU10x `booter_load` 在一张 GA102 CMP 90HX 上加载、SS0/SS1 PLM 写成功，同时自我限定写入的值 "were not right"（不对）、警告一次阳性测试不够。对 GA102 booter 的另一次独立静态分析得出结论：因为大小被严格验证，不存在溢出点。没人跑过决定性测试：在 GA102 上加载 TU10x booter 并尝试一次带回读的已知良好单 PLM 写。

> [!NOTE]
> **Windows 和非 Linux**
>
> 漏洞在 GPU 固件里、不依赖 OS。当前实现是 Linux，而 Unix 主机或开源驱动都不是硬要求，但一个 Windows 移植被描述为远不止几行工作。

> [!NOTE]
> **恢复一个 `csecret`**
>
> 三个索引映射到三个能力：`secret(6)` 解密 ECB 固件块（会产出 121.7 KB 明文固件加 Booter 代码）；`secret(2)` 伪造内容 MAC（那个路线的 CFG1 显存解锁和 PCIe 速度解锁两者的前提）；`secret(0)` 是启用带 `SKIP_VBIOS_SIG` 的 HULK 证书的调试绕过。**没有 csecret 被恢复过。** 三个都仍是需要电压毛刺硬件的差分故障分析目标。没有 **Booter 解密密钥**，加密的 booter 无法重建（只能经调试密钥路线读取）；没有 **VBIOS 调试密钥**，VBIOS 无法重新签名或跑调试模式。当前解锁通过把出厂的签名 booter 复用作执行引擎来绕开两者。
...
- **`envytools` 无法佐证任何这些。** 它的 Falcon 密码页有 Introduction、IO registers、Interrupts、"Submitting crypto commands: ccmd"、"Code authentication control" 和 "Crypto xfer control" 的小节标题，**每个都标着 "Todo: write me"**。没有关于 AES 引擎、密钥处理、签名代码认证、安全模式进出、代码页签名检查或任何 CMAC/CBC-MAC 方案的文档。记录下来以免有人再搜。envytools 也只把 Falcon 硬件记录到 v5（GK208+）、不含任何 Ampere 或 GA100 覆盖；它的寄存器图（`UC_CTRL 0x100`、`UC_ENTRY 0x104`、`UC_CAPS 0x108`、`UC_STATUS 0x128`、`CODE_INDEX 0x180`、`CODE 0x184`、`DATA_INDEX[0-7] 0x1c0`、`SCRATCH0 0x040`）只是结构背景、**必须不能**用来验证 GA100 寄存器地址。
- **通过返回到 IMEM `0x100` 的 `_start` 重新进入 booter**、带连续两个签名，被测试并以邮箱里同样的 `0x31` 失败。`0x00` 处的轻度安全引导程序在 Falcon 进入 HS 模式时被抹掉，所以没有可返回的东西。
- **通过跳过 `secure_teardown` 收获活 SCP 机密** 在提出的当天就被两条对抗性逐字节静态迹线反驳：`0x107`-`0x147` 处的序言把每个机密立即自 XOR 成零、真正的密钥使用是 `0x1e20`-`0x1e70` 处的 AES 验证、`0x1e74`-`0x206e` 处的清理扫描连续跑三次背靠背自清零、最后一个密码操作是 `0x206e cxor $c0, $c0`。从 `0x2070` 到 `0x7eef` 有**零**个密码操作，而劫持点（`0x37b3` 处 `lcall 0x4d4`）恰好坐在那个密码静默缺口内。跳过节省不了什么，因为寄存器组在大约 0x1500 字节更早的代码处就已经空了。
