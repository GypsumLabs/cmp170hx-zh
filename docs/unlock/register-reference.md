...
| `0x001180f8` | `NV_PGC6_BSI_SECURE_SCRATCH_14` | 位 26 = `BOOT_STAGE_3_HANDOFF`（INIT 0，DONE 1） | 由 SEC2 在 HS 上下文里在 GPU 侧设置；主机驱动只轮询它。引导挂起 `0x65` 就是那次轮询超时。Booter 自己的入口检查要求位 [31:28] == 0（否则错误 `0x29`）且位 [27:24] == 0（否则 `0x88`）。**已发布的链从不写这个寄存器。** |
...
>
> `0x11000000`、`0x13100000` 和 `0x17100000` 都被提出过。`0x17100000` 被测量同时满足 Booter 的 `0x29` 检查和主机 DONE 轮询，但写它也导致 `FBPA_008` 和 `FBPA_00C` 两者的 Booter PLM 打开失败。已发布的链通过从不碰该寄存器绕开整个问题。见[未解问题](../frontier/open-questions.md)。
...
| 地址 | 名称 | 功能 | 出厂 | 被谁碰 |
|---|---|---|---|---|
| `0x001fa7c4` | `WPR_PLM` | WPR 区域寄存器上的权限掩码 | `0x0004cb8f` | 已发布的补丁 0001、PLM 索引 2、打开到 `0xffffffff` |
| `0x001fa7c8` | `MMU_LOCK` PLM | 写半字节 `0x8` = 仅 L3/HS | `0x0004cb8f` | 只读 |
| `0x001fa7cc` | `WPR_CFG_PLM`（WPR 掩码 PLM） | WPR 允许掩码上的权限掩码 | `0x0004cb8f` | 已发布的补丁 0001、PLM 索引 0、打开到 **`0xfffff0ff`**、不是 `0xffffffff` |
...
| `0x001fa824` | `WPR2_ADDR_LO` | 位 [31:4] 里的值，`<< 12` | 空/INIT = `0x0fffffff` | **被已发布的补丁在每次 Booter 尝试前保存并重新武装**；净室 teardown 值是 `0x1ffffe00` |
...
### 被已发布的 `master` 写（四条目、按此顺序、每个最多两次尝试）
...
| `0x008200fc` | `OPT_PLM`（别名：`FUSE_SS_PLM`） | 一次扫描里在出厂卡上已读 `0xffffffff`、另一次读 `0x000003ff`；**永不被已发布的 master 写** |
...
**这些没有一个把 FB 几何布局移进常电域。** 一个专门的存活脚本（`geo_flr_survival_map_20260716.sh`）打开了全部六个加 `FUSE_SS_PLM`，发现 CFG1、CSTATUS 和 LMR 仍在 FLR 时回退。那个否定结果，正是已发布的设计在每次模块加载时、于 GSP 引导路径内重新应用几何布局、而不是尝试让它粘住的原因。
...
| `0x0000020B` | 32 | 11 | 65536 MiB | 已发布的 8 GB 档位 |
| `0x0000028A` | 40 | 10 | 40960 MiB | 已发布的 10 GB 档位 |
...
| `0x009a0148` | **FBPA PLM** | `0xffffff8f` | 被已发布的补丁 0001 打开到 `0xffffffff` |
...
**在已发布的驱动路径里一次广播写就够了**，而已发布的驱动完全不写任何每-FBPA 寄存器。在一个没有 devinit 的无驱动运行时里，广播单独不移动 CSTATUS，全部 24 个实例都必须手写。广播是 PRI 特权环硬件机制还是软件步骤，从未被直接插桩。
...
| `0x00823804` | **`FEAT_OVR_PLM`** | `0xffffff8f` | `0xffffffff` | 是一个 PLM | **挺过**（AON 岛） | 已发布的补丁 0001、PLM 索引 3 |
...
| `0x0082381c` | **`FEAT_OVR_SM_SPEED_SELECT`（SS0）** | 因卡而异：`0x53540175`、`0x12103060` | `0x88888888` | 是、经 `0x00823804` | **挺过** | 已发布的补丁 0001 |
| `0x00823820` | **`FEAT_OVR_SM_SPEED_SELECT_1`（SS1）** | 因卡而异：`0x00000000`、`0x00000003` | `0x00000008` | 是 | **挺过** | 已发布的补丁 0001 |
...
>
> 按晶片且无法解释。实测：`0x00100183`（出厂、PLM 范围扫描、中等）、`0x00000081`（解锁后探测、中等）、`0x00000181` / `0x00000182`（两块物理 170HX 单元、高、13 个分级差异之一）、`0x01000282`（A100 80 GB）。只读。**开放问题：** 为什么该值跨三份转储都不同；解锁或驱动里的某个东西可能在碰 Quadro-对-消费级分类字，那可能是驱动可见功能类的杠杆。下一步：在一张卡上于已发布的序列每个阶段前后重读这个寄存器。
...
> **未解问题：`0x00820520` 可写吗？**
>
> 一份分析把位 25 标注 "(writable)"；一个净室工具作为一条工作 Gen2 链的一部分把 `0x00200000` 写给它；PCIe 实地手册把它列为只读；而已发布的 Gen2 补丁只读它。没人发布过一次写后回读。Gen4 反正不可测试，因为没有正在做它的人有 Gen4 主机。见[Gen3 和 Gen4](../frontier/pcie-gen3-gen4.md)。
...
| `0x00820840` | MIG 使能 | 出厂 `0`；设位 0 启用 MIG、被报告为持久 | **不在已发布的树里**；一次全仓库 grep `0x820840` 返回空 |
...
本节一切**都是未发布分支材料**。已发布的 `master` 只含补丁 `0001` 到 `0006`、`constants.yaml` 里没有 `pcie:` 块，而对其安装器、卸载器、README 和构建脚本对 `gen2|gen 2|pcie|iommu|retrain|RMPcieLinkSpeed` 的一次不区分大小写 grep 返回零命中。见[PCIe Gen2](pcie-gen2.md) 和[PCIe 子系统](../hardware/pcie-subsystem.md)。
...
已发布的树里没有任何东西碰这些。一次全仓库 grep `0x504204`、`0x8200fc`、`0x82038c`、`0x8203f0`、`0x823818`、`0x820224`、`0x82059c` 和 `0x820840` 返回零命中。
...
这张表在已发布的 `master` 和全部十二个归档分支里**逐字节相同**（用校验和与一次 `0xc0deca7eU` 的 grep 验证，它在每份副本里恰好出现五次）。
...
>
> `0x0ccb` 是 `regtable_rw_indexed`，它索引的正是载荷粉碎的 DMEM `0x2383` 和 `0x8e08` 处的描述符表，而一次 2026-07-06 隔离矩阵显示每条携带写入的重接链都死在 `0xccb`。已发布的载荷却把 `0x00000ccb` 种在 `0xfff4`、解锁可演示地工作。通过追踪展开期间 `0xfff4` 是否曾被载入 PC、还是只是一个永不被返回穿过的帧里活过的保存槽来定论。
...
|---|---|---|---|---|
| 已发布的 `master` 和全部 12 个分支 | `0xf800` = 63,488 B | `0x0800` | `0xc0deca7e` | `0x6340`、`0xff58`、`0xff94`、`0xffa0`、`0xffc4` |
...
| `0x00240036`、`0x00340036` | `PL_LINK_RATE` 值（已发布的 Gen2 那个，和什么都没训练成的 Gen3 能力那个） |
...
这是塑造整个项目的那种不对称：算力比显存早几周发布，因为算力状态在常电岛里、显存状态不在。
