...
补丁集发出的几乎每一行都带那个前缀。唯一例外是补丁 0001 降级出的 WPR2 警告 `WPR2 already up before GSP boot; continuing for recovery`，它在 `LEVEL_WARNING` 打印、不带 `SEC2_DEBUG:` 标签。
...
脚本在 `set -euo pipefail` 下运行，所以单个被拒的 hunk 就会中止构建。因为循环是 `driver/patches/*.patch` 的一个普通 glob，一个第三方 diff 作为 `0007-*.patch` 放进去会和系列干净组合。那个的唯一记录案例见[P2P](../frontier/p2p.md)。
...
- 任何其它 GA100 SKU，包括同一机箱里的 A100，在装上打过补丁的模块时都会走完全出厂的路径。补丁在它上面是惰性的。
- `install.sh` 对 `lspci` grep `10de:20b0|10de:20c2|10de:2082`，所以一张 `20b0` 卡能成功安装、然后永不解锁，因为驱动内门不列出 `0x20B0`。安装器也这么说了：`This card reports 0x${DEVID}; install will continue, but unlock may not activate.`
- 补丁 0006 是 `>> 16` 形式的唯一例外。它在 `kernel-open/nvidia/nv.c` 里、PCI 探测时、在 `OBJGPU` 存在之前运行，所以它比较原始的 `nv->pci_info.device_id`。

几何布局在**运行时从设备 ID** 选择，而非在安装时。两个档位都烘焙进补丁 0001，所以安装器上的 `--profile=` 只改打印的横幅和元数据文件。见[显存几何布局](memory-geometry.md)。

---

## 0001 `sec2-postbl-plm-ss-cfg`：利用

这才是那个补丁。其它一切都是支持。它有 19,278 字节、约 460 行 diff，做七个不同的改动。
...
`_kgspCreateSignatureMemdesc` 通常分配 `NV_ALIGN_UP(pGspFw->signatureSize, 256)` 字节，实际操作是 4096。设备门为真时，它改为在 `ADDR_SYSMEM` 分配 `SEC2_POSTBL_TIMING_SIGNATURE_SIZE = 0x0000f800ULL`（63,488 字节）、按 256 字节对齐，因为这是 SEC2 Booter 的 DMA 引擎的要求。

那个超大缓冲区就是整个利用载体：已签名的 Booter 用一次无界 DMA 读它，并砸掉自己的栈。机制在[ROP 链](rop-chain.md) 和[Falcon 与 Booter](falcon-and-booter.md) 覆盖。
...
24 个里只有两个在趟之间变化：`0xf76c` 处的 `writeAddr` 和 `0xf754` 处的 `writeValue`。这条链是一个通用的单寄存器写原语，每个目标各重发一次。
...
四个权限级别掩码，每个最多两次尝试。PLM 是一个按寄存器的访问控制寄存器：它决定哪个权限级别可以读或写它守护的寄存器。见[权限级别掩码](privilege-level-masks.md)。
...
每次尝试周围，驱动保存并恢复写保护区域 2 的边界。循环前读 `wpr2Lo = GPU_REG_RD32(pGpu, 0x001fa824U)` 和 `wpr2Hi = GPU_REG_RD32(pGpu, 0x001fa828U)`；每次尝试写回两者、为那个 `{address, value}` 对重新填充载荷，然后调用：
...
并回读目标寄存器。循环后它最后再恢复一次 `wpr2Lo`/`wpr2Hi`。最坏情况是**正常引导 Booter Load 之前的八次 Booter Load 执行**。

> [!NOTE]
> **每次载荷趟的 `status=0xffff` 是预期的**
>
> `s_executeBooterUcode_TU102` 在每次运行后会从 seccode 留在 mailbox0 里的一个错误码看到失败，所以 `kgspExecuteBooterLoad_HAL` 在载荷趟上无条件返回 `NV_ERR_GENERIC`（`0xffff`）。那是注入链已经执行*之后*提出的无效签名抱怨。**寄存器回读是唯一有效的成功标准。**
> `kgspExecuteBooterLoad_TU102` 还在每次运行前执行 `kflcnReset(SEC2)`，所以 SEC2 在趟之间不累积状态。这些早期趟期间的 Booter 状态 `0x31` 也被容忍。
...
> **文档分支对 SS0 和 SS1 是错的**
>
> `docs/ARCHITECTURE.md` 声称 cmpunlocker 对 SS0 和 SS1 都写 `0xffffffff`、并显示匹配的预期 dmesg 行。它并不写。对着那些字符串验证一次解锁，会把一张工作卡看起来像坏了。代码自 2026-07-18 起就写 `0x88888888` / `0x00000008`。见[算力节流](compute-throttle.md)。
...
`kgspSec2PostblTimingRebuildStockSignature()` 释放 `pSignatureMemdesc`，用 `MEMDESC_FLAGS_ALLOC_IN_UNPROTECTED_MEMORY` 在 `NV_ALIGN_UP(stockSignatureSize, 256)` 重新创建它，把保存的出厂签名复制回来，把 `pWprMeta->sysmemAddrOfSignature` 和 `pWprMeta->sizeOfSignature` 重置为新描述符。如果它返回任何非 `NV_OK`，`_kgspBootGspRm` 会传播那个状态、引导以 `SEC2_DEBUG: rebuild stock signature failed: 0x%x` 中止。
...
执行直落进 `kgspPopulateWprMeta_HAL`。这是强制性的：解锁会运行 Booter Load 最多八次，而 Booter Load 让 WPR2 保持 up。
...
**它打印决定性的证明行。** 真实的 `kgspExecuteBooterLoad_HAL` 之后，只对两个设备 ID，打印 `SEC2_DEBUG: normal BooterLoad status=0x%x`，然后——只有当那个状态是 `NV_OK` 时：
...
控制流实际上不变：替换物仍在任何非 `NV_OK` 结果时 `return status`，从 FWSEC 或两个 Falcon 复位处返回，正如断言那样。0002 买到的是诊断。没有它，没有任何东西命名四个步骤里哪个失败、带什么状态，而唯一的引导后验证行消失——那是每个排障流程都要的那一行。见[验证](../procedures/verify.md)。

---

## 0003 `late-pma`：让内存可分配

补丁 0001 告诉 GSP-RM 帧缓冲更大，但那不等于让额外内存可分配。物理内存分配器（PMA）仍必须被告知。补丁 0003 有**四个 hunk** 加一个钩子。
...
额外帧缓冲被报告、却从不交给 PMA，所以区域保持保留、高于出厂大小的分配失败。这就是 "nvidia-smi shows 65536 MiB" 和 "you can actually allocate 63 GB"（你真能分配 63 GB）之间的区别。

---

## 0004 `bar0-pramin-clamp`：让 PRAMIN 可达

最小也最有趣的补丁：`src/nvidia/src/kernel/gpu/bus/arch/maxwell/kern_bus_gm107.c` 里一个十行的 hunk。PRAMIN 是 BAR0 里一个 CPU 经它到达帧缓冲内存的滑动窗口。出厂代码把它放在帧缓冲顶部：
...
PRAMIN 孔径从 65536 MB 计算、落出可达 BAR0 空间。GA100 BAR0 是一个 16 MiB 的 PRI 孔径；放在 64 GiB 地址空间顶部的一个窗口根本无法经它寻址。
...
清理器用一个可压缩的 PTE kind、跨一个 MMU 表并非为之构建的帧缓冲，尝试虚拟模式复制引擎操作。这是你最可能想跳过、却绝不该跳过的补丁。
...
两个分支相同、能合并成一个条件。是美观问题，不是 bug。

`NV_FLAG_PERSISTENT_SW_STATE` 是一个为 SR-IOV 虚拟功能而存在的 `nv.h` 标志。在这里改作他用：它阻止 RM 在最后一个客户端关闭时拆除软件状态。它实际就是内置持久化模式，也是当前设计不需要 systemd 看门狗的原因。更早几代的工具确实需要一个；`remove.sh` 仍清理当前安装器从不创建的遗留 `cmpunlocker.service` 和 `/opt/cmpunlocker/daemon/watchdog.py`。见[工具谱系](../history/tool-lineage.md)。
...
补丁用虚构的 NVIDIA 风格名字伪装自己。`SEC2_DEBUG_PRI_*`、`kgspSec2PostblTiming*` 和 `SEC2_DEBUG:` 日志前缀在出厂 610.43.03 源码里**任何地方都不出现**。"PostBL Timing" 是一个听上去合理、却是虚构的功能名。两次独立审查把这读成：把利用代码刻意伪装成制造或调试功能，并作为反 NVIDIA 内部作者身份的证据——因为一个有合法内部访问权的人不需要伪装。实际后果是有用的：单次 grep 就在引导日志里找到每一条解锁行。
