...
> **这会废除一切并可能丢失数据**
>
> 打过补丁的内核模块未签名，所以安全启动必须关闭、内核被污染。给解锁的卡超频会在不崩溃的情况下静默损坏内存。10 GB 卡上的 80 GB 配置报告的是它无法可靠交付的容量。在运行任何超出出厂档位的配置之前，请先读[Risks](../start/risks.md) 和[调优](../operations/tuning.md)。
...
> **永远不要把档位搞混**
>
> 8 GB 卡解锁到 64 GB，10 GB 卡解锁到 40 GB。把 8 GB 几何布局应用到 10 GB 卡上是有据可查的失败模式，而 10 GB 卡的 80 GB 配置也已被尝试并证实不稳定。见[显存几何布局](memory-geometry.md)。
...
| `0004-bar0-pramin-clamp.patch` | 861 B | 让 PRAMIN 窗口保持在 BAR0 可达空间内 |
...
> **实验性**
>
> **`clanker_driver-port`** 分支添加 `580/`、`590/`、`595/` 和 `610/` 补丁目录。每个寄存器值和载荷偏移量都与 `master` 逐字符相同，而 `610` 目录是它的一份逐字节副本。595 / 590 / 580 移植**仅做过源码验证**：补丁可干净应用，但无人报告过启动。

> [!WARNING]
> **`memory` 分支是单设备并硬编码 8 GB 档位**
>
> `memory` 先于双几何布局支持而存在。它的补丁 `0001` 硬编码 `SEC2_POSTBL_TIMING_CMP_170HX_PCI_DEVICE_ID 0x20C2`、`cfg1Value = 0x02779000U` 和 `lmrValue = 0x0000020BU`，既没有设备 ID 分支，也没有 10 GB 路径：`0x2082` 卡在那个分支上根本不会被解锁。别指望运行时档位选择而从它构建。

`ecc` 分支根本不含任何 ECC 实现：全部六个驱动补丁都与 `master` 逐字节相同。`docs` 分支的 `ARCHITECTURE.md` 是一个文档缺陷，不应被引用：它把 SS0/SS1 称作 "Suspension State" 寄存器、声称两者都被写成 `0xffffffff`、把 PLM 展开为 "Program Logic Modules"，还打印出代码中根本不存在的日志行。

## 解锁买到什么，实测

标注 2026-07-06 的行来自一张随第一份私有 "compute unlock working" 报告一并贴出的单个渲染映像，而非来自具名工具的输出。出处注意事项见[compute-throttle.md](compute-throttle.md)。
...
| INT8（2026-07-06，一张卡） | 1.63 TOP/s | 50.50 TOP/s | 30.9x。一场更晚的 8 租用卡行动（2026-07-27）在库路径上测得 44.1 TOPS，但没有匹配的锁定基线；INT8 *张量*路径测得 335 TOPS |
| FP16 标量（非张量） | 约 42-50 TFLOPS | 不变 | 从未被节流，这正是锁定卡当时已可用于 token 生成的原因 |
...
成功信号是 `0x00823818` 处的 `FEAT_READOUT_1` 读回 `0x00000000`。出厂 170HX 读回 `0x016db6ed`。这个寄存器是判断"这张卡是否已解锁"最干净可用的判据。

> [!NOTE]
> **未解问题**
>
> 解锁后 INT8 / IMMA 仍被门控，尽管 IMLA 覆盖半字节与 FMLA、FFMA 的设置方式相同。在 A100 上，INT8 大约比 FP16 快 2 倍；在解锁的 170HX 上则慢 3.7 倍。对推理的实际影响：使用 W4A16（AWQ、GPTQ），完全避开 W8A8。见[LLM 推理](../operations/llm-inference.md)。

## 它为什么能工作

三个事实支撑一切：

1. **主灭杀熔丝未烧断。** `0x008203f0` 处的 `OPT_FEATURE_FUSES_OVERRIDE_DISABLE` 在 CMP 170HX 上读回 `0x00000000`。若它被烧断，所有特性覆盖都会永久锁定，也不存在任何软件路径。
...
3. **调试和量产 booter 映像包含相同的明文代码。** 只有 AES 密钥不同，而调试密钥是一把非机密的编号测试密钥，所以无需任何泄露源码即可读取量产 HS 代码。

该利用是对一个厂商签名 blob 的纯数据攻击，其验证密钥熔入不可变的硅片，因此这个脆弱的 booter 无法通过驱动更新来吊销。这是对信任模型的推断，而非已演示的结果。
...
| [寄存器参考](register-reference.md) | 每个地址、每个实测值，按 SKU 与按对比部件 |
