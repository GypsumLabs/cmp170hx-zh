...
1400 MHz 上限下最高的 *验证过* 点是 **+250 到 +300**（+300 通过 4 次扫描、0 错误）。配套 GFLOP/W 表的 1390 GFLOP/W 峰值在 1400/+350，它从未被扫描验证，且坐在两个失败之间（+325 损坏、+375 故障）。1350/+300 处的 1376 GFLOP/W 格同类：一次完成运行，从不被模式扫描把关。同一个 1350 阶梯也含 1350/+400，它通过两次扫描，随后一次返回 `mem_errors=1`。最高的 **扫描验证过** 效率点是已发布的 `eff` 档位：**+250 / 1350 MHz 处 1366 GFLOP/W**（180.3 TFLOPS、132.0 W）。1650 上限下效率跑 +250 处 1067 GFLOP/W 到 +350 处约 1149 GFLOP/W；那里更高的数字只来自故障的偏移。`eff` 档位是已发布的默认，因为它用三分之一的功耗换来 2% 的吞吐。
...
- 已发布的 `master` 把两个设备 ID 的 BAR0/PRAMIN 窗口钳到 8 GiB（`0004-bar0-pramin-clamp.patch`）。这是一个 CPU 侧孔径、不是设备侧分配限制，但它是基于 PRAMIN 的工具只看到前 8 GiB 的原因。
...
| 最佳 *扫描验证过* GFLOPS/W | 1366 GFLOP/W | `eff`（已发布的默认）、+250 / 1350 MHz、132.0 W 下 180.3 TFLOPS；至少两次以 `mem_errors=0` 通过全-VRAM 模式扫描 |
